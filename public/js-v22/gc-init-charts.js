...
*/
    ...
    // -------------------------------------------------------------------------------------------------------------------
    // -------------------------------------------------------------------------------------------------------------------
    chart_dashboard_co2_budget = createCo2BudgetChart ('container_dashboard_co2_budget');
...
function initClimateCharts () {
  ...
  // alert ("initClimateCharts()");
    ...
    chart_temperature_timeseries = new Highcharts.Chart({
        ...
        title: {
            style: { font: 'normal 11px "Trebuchet MS", Verdana, sans-serif' },
  	    text: 'Global mean temperature change'
        },
        legend: { enabled: false },
        exporting: { enabled: false },
        plotOptions: {
...
*/
       ...
       ren.text('observed', x1, 60)
         .css({color: '#C0C0C0', fontSize: '9px'}).add();

       x1 = chart.xAxis[0].toPixels(2013) + 10;

       ren.text('projected', x1, 60)
         .css({color: '#C0C0C0', fontSize: '9px'}).add();
...
function initManufacturingEnergyUseChart (container, title, yMax, yTick) {
    ...
    var chart = new Highcharts.Chart({
	  ...
	  name: 'Consumables e.g. appliances, packaging',
          marker: { radius: 2 },
          color: '#e78ac3',
          data: [ [2011, 1], [2015, 1], [2020, 1], [2025, 1], [2030, 1], [2035, 1], [2040, 1], [2045, 1], [2050, 1] ]
         }, {
...
function initFoodCharts () {
  ...
  var unit = "grams / day";
  if (dietTableUnit == 1) unit = "grams/day";
  else                    unit = "kcal/day";
  ...
  var chart = new Highcharts.Chart({
       ...
       chart: {
        ...
        series: [{
            ...
            innerSize: 50,
            data: [
                ['consumed '  + variable,  50.0],
                ['available ' + variable,  50.0]
            ]
        }]
    });
...
function initLandGrainFedCharts() {
  ...
  chart_land_yields_grain_fed = new Highcharts.Chart({
    ...
    title: {
      style: { font: 'normal 11px "Trebuchet MS", Verdana, sans-serif' },
      text: 'Livestock fed with cereals & residues'
    },
    legend: { enabled: false },
    exporting: { enabled: false },
  ...
  // var titleX =  dodo('Land use');
    ...
    chart_land_overview = new Highcharts.Chart({
        ...
        tooltip: {
          ...
          // headerFormat: '{point.key}' + ' (' + dodo('M ha') + ')' + '<table>',
          pointFormat: '<tr><td style="color: {series.color}">{series.name}: </td>' +
          '<td style="text-align: right">{point.y}</td></tr>',
          footerFormat: '</table>'
        },
